...
namespace SLDP
{
	...
	void GUITrack::modifyTrack(Track& track) const
	{
		for (size_t i = 0; i < edges.size(); ++i)
		{
			...
			edge->setWeight(edges[i]->isBlocked() ? INT_MAX : edge->getWeight());
		}
		for(size_t i = 0; i < switches.size(); ++i)
		{
			GUINode* n = switches[i]->getNodes()[0];
			track.getFirstNode(n->getLabel())->setCurrentEdge(RIGHT, n->isSwitchedRight());
		}

		
		track.getFirstNode("Three")->setCurrentEdge(RIGHT, getFirstNode("Three")->isSwitchedRight());
		track.getFirstNode("Four")->setCurrentEdge(RIGHT, getFirstNode("Four")->isSwitchedRight());
		track.getFirstNode("Six")->setCurrentEdge(LEFT, getFirstNode("Six")->isSwitchedLeft());
		track.getFirstNode("Eight")->setCurrentEdge(LEFT, getFirstNode("Eight")->isSwitchedLeft());
	}
	...
	GUINode* GUITrack::getFirstNode(const std::string& label) const
	{
